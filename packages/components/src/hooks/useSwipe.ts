...
const useSwipe = <TElement extends HTMLElement>(
  ...
  const {onMouseDown, onMouseMove, onMouseLeave, onMouseUp} =
    useMouseEvents<TElement>(targetRef, opts.passive)
  const {onTouchStart, onTouchMove, onTouchEnd, onTouchCancel} =
    useTouchEvents<TElement>(targetRef, opts.passive)
  ...
  const continueSwipe = (event: MouseEvent | TouchEvent) => {
    ...
    if (
      ...
      if (
        opts.threshold &&
        opts.direction === 'both' &&
        (Math.abs(alpha[0]) > opts.threshold ||
          Math.abs(alpha[1]) > opts.threshold)
      ) {
